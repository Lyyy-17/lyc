...
1. 仅用单个 NetCDF 构建窗口样本（不走 manifest）。
2. 单次前向不足目标时长时，滚动回灌预测。
3. 统计整体与各预测步指标，与阈值比对（PASS/FAIL）。
4. 输出 JSON / CSV / Markdown 与可视化图。

示例::

  python scripts/06_element_assess.py \\
...
def main() -> None:
    ...
    ap.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/element_forecasting/competition_single_file",
    )
    args = ap.parse_args()

    log_file = ROOT / "outputs/logs/element_assess.log"
