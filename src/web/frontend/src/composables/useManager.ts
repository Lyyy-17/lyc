...
import { useEddy } from './useEddy'

const { anomalyData, anomalyProduct } = useAnomaly()
const { eddyResult } = useEddy()
...
const managerRiskSnapshot = computed(() => {
  if (anomalyData.value) {
    ...
    return {
      ...
      highRiskZoneCount: p.typhoon.zones.filter((z) => z.score >= 60).length,
...
const managerZoneResponsibilities = computed(() => {
  ...
  return baseZones.map((z) => ({
    ...z,
    agency: (managerAgencyByZone as Record<string, string>)[z.name] || '海洋综合值班中心',
...
const managerBriefContent = computed(() => {
  ...
  const zoneLines = managerZoneResponsibilities.value
    .map((z) => `- ${z.name}: 风险分${z.score} | 责任单位 ${z.agency} | 建议 ${z.action}`)
    .join('\n')
