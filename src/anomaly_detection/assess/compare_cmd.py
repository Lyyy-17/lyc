"""子命令 compare：主 AE / AE 基线 / PCA / IsolationForest 统一对比。"""
...
def register_parser(sub: argparse.Action) -> argparse.ArgumentParser:
    p = sub.add_parser("compare", help="统一对比主 AE、AE baseline、PCA、IsolationForest")
    ...
    p.add_argument("--iforest-contamination", type=float, default=0.05)
    p.add_argument("--random-state", type=int, default=42)
    p.add_argument("--main-summary", default="outputs/anomaly_detection/summary.json")
    p.add_argument("--main-split-reports", default="outputs/anomaly_detection/split_reports.json")
    p.add_argument("--baseline-summary", default="outputs/baseline/anomaly_detection/summary.json")
    p.add_argument("--baseline-split-reports", default="outputs/baseline/anomaly_detection/split_reports.json")
    p.add_argument("--output-dir", default="outputs/baseline/anomaly_detection_traditional")
    return p
...
def run(args: argparse.Namespace, root: Path) -> None:
    ...
    labels = _load_labels(
        resolve_path(args.labels_json, root=root, default=root / "outputs/anomaly_detection/labels.json")
        if args.labels_json
        else None
    )

    train_ds = _subset_if_needed(
        _build_dataset("train", processed_dir, manifest, norm_stats, lru, root),
        args.max_train_samples,
    )
    val_ds = _subset_if_needed(
        _build_dataset("val", processed_dir, manifest, norm_stats, lru, root),
        args.max_val_samples,
    )
    test_ds = _subset_if_needed(
        _build_dataset("test", processed_dir, manifest, norm_stats, lru, root),
    ...
    out_dir = resolve_path(args.output_dir, root=root, default=root / "outputs/baseline/anomaly_detection_traditional")
    ...
    main_summary = _read_json_if_exists(resolve_path(args.main_summary, root=root, default=root / "outputs/anomaly_detection/summary.json"))
    main_reports = _read_json_if_exists(resolve_path(args.main_split_reports, root=root, default=root / "outputs/anomaly_detection/split_reports.json"))
    base_summary = _read_json_if_exists(resolve_path(args.baseline_summary, root=root, default=root / "outputs/baseline/anomaly_detection/summary.json"))
    base_reports = _read_json_if_exists(resolve_path(args.baseline_split_reports, root=root, default=root / "outputs/baseline/anomaly_detection/split_reports.json"))
